...
public class PlayGameResponseParser extends AResponseParser {
    ...
    public PlayGameResponseParser(TcpClient client,
        ...
        messageError = Protocol.MSG_PLAY_GAME.getErrorType(MESSAGE.getNextArg());
    }
    ...
    public String getStatusAndUpdateGUI() {
        ...
        if (messageError.equals(Protocol.MSG_ERR_ROUND_NOT_STARTED)) {
            return "Hráč nemůže táhnout před zahájením kola";
        }
        
        if (messageError.equals(Protocol.MSG_ERR_CANNOT_PLAY_IN_ROUND)) {
            return "Hráč již nemůže táhnout v tomto kole";
        }
        
        if (messageError.equals(Protocol.MSG_ERR_CANNOT_PLAY_NOW)) {
            return "Hráč není na tahu";
        }
        
        if (messageError.equals(Protocol.MSG_ERR_INVALID_POSITION)) {
            return "Hráč provedl tah na neplatnou pozici";
        }
        
        if (messageError.equals(Protocol.MSG_ERR_CELL_OUT_OF_BOARD)) {
            return "Hráč provedl tah na pozici mimo hranice hracího pole";
        }
        
        if (messageError.equals(Protocol.MSG_ERR_CELL_OCCUPIED)) {
